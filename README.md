...
| **제품** | 모바일 앱 2종 배포 · 웹 서비스 MVP~운영 · 3D 인터랙션 프로토타입 · **Process OS 레퍼런스** ([Goodz](https://github.com/dayainow/goodz)) |
...
| [**API Layer Codegen MCP**](https://github.com/dayainow/dayainow/tree/main/api-layer-codegen-mcp) | OpenAPI 탐지 → TypeScript/Zod/TanStack Query API 레이어 생성까지 stdio 하네스로 검증하는 MCP 서버 | TypeScript · MCP · OpenAPI · Zod · TanStack Query | [소스](https://github.com/dayainow/dayainow/tree/main/api-layer-codegen-mcp) |
...
| [**Goodz**](https://github.com/dayainow/goodz) | 굿즈 이커머스 풀스택 — 기획 입력·변경 로그·산출물 레지스트리·승인 로그까지 대시보드로 추적하는 **Process OS 레퍼런스** | Turborepo · pnpm · Next.js · Express · Process Dashboard · GHA CI | [GitHub](https://github.com/dayainow/goodz) |
...
| [**Frontend Agent Orchestrator Kit**](https://github.com/dayainow/frontend-agent-orchestrator-kit) | 기획/디자인 → 컴포넌트 → Mocking → 테스트 → 배포 검증을 묶는 프론트엔드 AI 에이전트 하네스 · 실전 적용: [Goodz Process OS](https://github.com/dayainow/goodz) |
...
- **End-to-end**: 기획(PRD)·디자인(Figma)·UI·API·QA·배포·출시 준비(약관·OG 등)까지 직접 수행 — [Goodz](https://github.com/dayainow/goodz)에서 Process OS(기획 입력·변경·산출물·승인)로 문서화
