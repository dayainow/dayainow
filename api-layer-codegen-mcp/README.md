...
OpenAPI/GraphQL/Postman 같은 API 소스를 탐지하고, TypeScript 타입, Zod 스키마, TanStack Query hook 또는 API 클라이언트로 된 프론트엔드 API 레이어를 생성하는 MCP 서버입니다.
...
아닙니다. 여러 API 소스를 받아 프론트엔드 API 레이어로 정리할 수 있습니다. 포맷마다 원본에 담긴 정보량이 달라 생성 결과의 깊이는 다릅니다. OpenAPI/Swagger는 schema 기반 타입까지 강하게 만들고, Postman/Insomnia/.http는 요청 컬렉션 기반 클라이언트를 만들며, AsyncAPI는 이벤트/메시지 클라이언트를 만듭니다.
...
| Swagger 2.0 | 지원 | 지원 | 내부 변환 후 TypeScript/Zod/TanStack Query 생성 |
| GraphQL Schema (`.graphql`, `.gql`) | 지원 | 지원 | SDL 기반 타입/Zod/GraphQL fetcher + TanStack Query 생성 |
| Postman Collection | 지원 | 지원 | collection item 기반 request client + TanStack Query 생성 |
| Insomnia Export | 지원 | 지원 | export request 기반 request client + TanStack Query 생성 |
| AsyncAPI | 지원 | 지원 | 메시지 타입/Zod/AsyncAPI publish client 생성 |
| `.http`, `.rest` request file | 지원 | 지원 | 요청 예시 기반 request client + TanStack Query 생성 |

구조적으로는 `detect_api_spec`가 다양한 API 소스를 찾고, `generate_code`가 포맷별 입력 어댑터를 통해 같은 출력 규칙(TypeScript, Zod, TanStack Query/API client)을 공유합니다.
...
현재 구현은 OpenAPI 3.x, Swagger 2.0, GraphQL SDL, Postman Collection, Insomnia Export, AsyncAPI, HTTP request 파일 생성을 지원합니다.
